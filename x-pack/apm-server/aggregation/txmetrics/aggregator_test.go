		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, m := range metricsets {
				...
				if len(m.Metricset.Samples) > 0 {
					...
					}, m, cmpopts.IgnoreTypes(netip.Addr{}), cmpopts.IgnoreFields(model.Service{}, "Name")))
				}
			}
			var expectedTotalOverflowGrps int
			if tc.expectedPerSvcTxnLimitOverflow > 0 {
				// since for this test we are overflowing all the services equally, each overflow will
				// account for 2 metricsets: 1 for the aggregated data and 1 for the overflow metric.
				if tc.uniqueServices > maxSvcs {
					expectedTotalOverflowGrps += maxSvcs * 2
				} else {
					expectedTotalOverflowGrps += tc.uniqueServices * 2
				}
			}
			if tc.expectedOtherSvcTxnLimitOverflow > 0 {
				// if the overflow spills into the other bucket then we will have 1 metricset for
				// aggregated data and another for the overflow metric.
				expectedTotalOverflowGrps += 2
			}
